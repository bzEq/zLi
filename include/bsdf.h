...
struct LambertianDiffuse: public BSDF {
  std::tuple< Float, boost::optional<Vector3f> > SampleDirection(const Vector3f& normal, const Vector3f& wo) const {
    if (normal * wo <= 0) return std::make_tuple(0, boost::optional<Vector3f>());
